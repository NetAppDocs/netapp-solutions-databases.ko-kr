...
summary: 이 솔루션은 FSx ONTAP 에 NFS 마운트를 사용하여 AWS EC2 컴퓨팅 인스턴스에 배포된 Oracle VLDB의 빠른 복구 및 복제에 대한 개요와 세부 정보를 제공하고, RMAN을 통해 지속적으로 증분 병합할 대기 데이터 파일 복사본을 준비합니다. 
---
= TR-4973: AWS FSx ONTAP 에서 증분 병합을 통한 Oracle VLDB의 빠른 복구 및 복제
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
이 솔루션은 FSx ONTAP 에 NFS 마운트를 사용하여 AWS EC2 컴퓨팅 인스턴스에 배포된 Oracle VLDB의 빠른 복구 및 복제에 대한 개요와 세부 정보를 제공하고, RMAN을 통해 지속적으로 증분 병합할 대기 데이터 파일 복사본을 준비합니다.



== 목적

Oracle Recovery Manager(RMAN) 백업 도구를 사용하여 Oracle에서 매우 큰 데이터베이스(VLDB)를 복구하는 것은 매우 어려운 작업이 될 수 있습니다.  장애 발생 시 백업 미디어에서 데이터베이스를 복원하는 과정은 시간이 많이 걸릴 수 있으며, 데이터베이스 복구가 지연되고 잠재적으로 서비스 수준 계약(SLA)에 상당한 영향을 미칠 수 있습니다.  그러나 Oracle은 10g 버전부터 사용자가 DB 서버 호스트에 있는 추가 디스크 스토리지에 Oracle 데이터베이스 데이터 파일의 단계적 이미지 복사본을 생성할 수 있는 RMAN 기능을 도입했습니다.  이러한 이미지 복사본은 RMAN을 사용하여 매일 증분적으로 업데이트할 수 있습니다.  장애가 발생한 경우, 데이터베이스 관리자(DBA)는 장애가 발생한 미디어에서 이미지 복사본으로 Oracle 데이터베이스를 신속하게 전환할 수 있으므로 데이터베이스 미디어를 완전히 복원할 필요가 없습니다.  결과적으로 SLA는 크게 향상되었지만, 필요한 데이터베이스 저장 용량은 두 배로 늘어나게 됩니다.

VLDB에 대한 SLA를 원하고 Oracle 데이터베이스를 AWS와 같은 퍼블릭 클라우드로 옮기는 것을 고려하고 있다면 AWS FSx ONTAP 과 같은 리소스를 사용하여 스탠바이 데이터베이스 이미지 복사본을 스테이징하는 유사한 데이터베이스 보호 구조를 설정할 수 있습니다.  이 문서에서는 AWS FSx ONTAP 에서 NFS 파일 시스템을 프로비저닝하고 내보내 Oracle 데이터베이스 서버에 마운트하여 기본 스토리지 장애 발생 시 신속한 복구를 위해 대기 데이터베이스 복사본을 준비하는 방법을 보여줍니다.

더 나아가, NetApp FlexClone 활용하여 추가 스토리지 투자 없이 동일한 대기 데이터베이스 이미지 복사본으로 개발/테스트 Oracle 환경을 구축하는 등의 다른 사용 사례를 위해 동일한 스테이징 NFS 파일 시스템의 복사본을 만드는 방법도 보여드립니다.

이 솔루션은 다음과 같은 사용 사례를 해결합니다.

* AWS FSx ONTAP 스토리지의 NFS 마운트 지점에서 RMAN을 통해 Oracle VLDB 이미지 복사 증분 병합.
* 장애 발생 시 FSx ONTAP 스토리지에 있는 데이터베이스 이미지 복사본으로 전환하여 Oracle VLDB를 빠르게 복구합니다.
* 다른 사용 사례를 위해 또 다른 데이터베이스 인스턴스를 구축하는 데 사용할 Oracle VLDB 이미지 복사본을 저장하는 FSx ONTAP NFS 파일 시스템 볼륨을 복제합니다.
...
* AWS에서 RMAN을 통해 Oracle VLDB 이미지 복사 증분 병합을 설정하여 데이터베이스 복구를 더 빠르게 한 DBA입니다.
* AWS 퍼블릭 클라우드에서 Oracle 워크로드를 테스트하는 데이터베이스 솔루션 아키텍트입니다.
* AWS FSx ONTAP 스토리지에 배포된 Oracle 데이터베이스를 관리하는 스토리지 관리자입니다.
* AWS FSx/EC2 환경에서 Oracle 데이터베이스를 구축하려는 애플리케이션 소유자입니다.
...
이 솔루션의 테스트와 검증은 최종 배포 환경과 일치하지 않을 수 있는 AWS FSx ONTAP 및 EC2 환경에서 수행되었습니다. 자세한 내용은 다음 섹션을 참조하세요. <<배포 고려사항의 핵심 요소>> .



=== 아키텍처

image:aws-ora-fsx-ec2-vldb-architecture.png["이 이미지는 FSx ONTAP 사용하여 AWS 퍼블릭 클라우드에서 Oracle VLDB 증분 병합을 구현하는 방법에 대한 자세한 그림을 제공합니다."]
...
* *RMAN 증분 병합을 위한 Oracle VLDB 저장 레이아웃.*  테스트와 검증에서 Oracle 증분 백업 및 병합을 위한 NFS 볼륨은 처리량이 4GBps, 원시 SSD IOPS가 160,000이고 용량 제한이 192TiB인 단일 FSx 파일 시스템에서 할당되었습니다.  임계값을 초과하여 배포하는 경우, 여러 FSx 파일 시스템을 여러 NFS 마운트 지점과 병렬로 연결하여 더 높은 용량을 제공할 수 있습니다.
* *RMAN 증분 병합을 사용한 Oracle 복구성.*  RMAN 증분 백업 및 병합은 일반적으로 RTO 및 RPO 목표에 따라 사용자가 정의한 빈도로 실행됩니다.  1차 데이터 저장소 및/또는 보관된 로그가 완전히 손실되면 데이터 손실이 발생할 수 있습니다.  Oracle 데이터베이스는 FSx 데이터베이스 백업 이미지 복사본에서 사용 가능한 마지막 증분 백업까지 복구할 수 있습니다.  데이터 손실을 최소화하기 위해 Oracle 플래시 복구 영역을 FSx NFS 마운트 지점에 설정하고 보관된 로그를 데이터베이스 이미지 복사본과 함께 FSx NFS 마운트에 백업할 수 있습니다.
* *FSx NFS 파일 시스템에서 Oracle VLDB 실행.*  데이터베이스 백업을 위한 다른 대용량 스토리지와 달리 AWS FSx ONTAP 높은 수준의 성능과 스토리지 효율성을 제공하는 클라우드 지원 프로덕션 등급 스토리지입니다.  Oracle VLDB가 기본 저장소에서 FSx ONTAP NFS 파일 시스템의 이미지 복사본으로 전환되면 기본 저장소 오류가 해결되는 동안에도 데이터베이스 성능을 높은 수준으로 유지할 수 있습니다.  기본 저장소 오류로 인해 사용자 애플리케이션 경험이 저하되지 않는다는 사실을 알면 안심할 수 있습니다.
* * 다른 사용 사례를 위한 NFS 볼륨의 FlexClone Oracle VLDB 이미지 복사본.*  AWS FSx ONTAP FlexClone 쓰기 가능한 동일한 NFS 데이터 볼륨의 공유 복사본을 제공합니다.  따라서 Oracle 데이터베이스가 전환될 때에도 Oracle VLDB 이미지 복사본의 스테이징 무결성을 유지하면서 다른 많은 사용 사례에 사용할 수 있습니다.  이를 통해 VLDB 스토리지 공간을 대폭 줄여 스토리지 비용을 엄청나게 절감할 수 있습니다.  NetApp Oracle 성능을 높은 수준으로 유지하기 위해 기본 스토리지에서 데이터베이스 이미지 복사본으로 데이터베이스를 전환하는 경우 FlexClone 활동을 최소화할 것을 권장합니다.
...
* *dNFS 구성.* dNFS는 Oracle 커널에 내장되어 있으며 Oracle을 NFS 스토리지에 배포할 때 Oracle 데이터베이스 성능을 획기적으로 높이는 것으로 알려져 있습니다. dNFS는 Oracle 바이너리에 패키징되어 있지만 기본적으로 켜져 있지 않습니다.  NFS에 Oracle 데이터베이스를 배포하는 경우 이 기능을 켜야 합니다.  VLDB에 여러 FSx 파일 시스템을 배포하는 경우, 다양한 FSx NFS 파일 시스템에 대한 dNFS 다중 경로를 적절히 구성해야 합니다.
...
VPC 내의 AWS EC2 환경에 Oracle VLDB가 이미 배포되어 있다고 가정합니다.  AWS에 Oracle을 배포하는 데 도움이 필요하면 다음 기술 보고서를 참조하세요.
...
Oracle VLDB는 FSx ONTAP 이나 AWS EC2 생태계 내의 다른 스토리지에서 실행할 수 있습니다.  다음 섹션에서는 AWS FSx ONTAP 스토리지의 NFS 마운트에 스테이징된 Oracle VLDB의 이미지 복사본에 대한 RMAN 증분 병합을 설정하는 단계별 배포 절차를 제공합니다.
...
. AWS EC2 콘솔에서 Amazon FSx ONTAP 스토리지 HA 클러스터를 배포하여 Oracle 데이터베이스 대기 이미지 복사본을 저장하는 NFS 볼륨을 호스팅합니다.  FSx 스토리지 배포에 익숙하지 않은 경우 설명서를 참조하세요.link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/creating-file-systems.html["FSx ONTAP 파일 시스템 생성"^] 단계별 지침을 확인하세요.
...
[%collapsible%open]
====
이 데모에서는 FSx 클러스터 관리 IP를 통해 ssh를 통해 fsxadmin 사용자로 FSx 클러스터에 로그인하여 명령줄에서 NFS 볼륨을 프로비저닝하는 방법을 보여드립니다.  또는 AWS FSx 콘솔을 사용하여 볼륨을 할당할 수도 있습니다.  데이터베이스 크기를 수용하기 위해 두 개 이상의 FSx 파일 시스템이 설정된 경우 다른 FSx 파일 시스템에서 절차를 반복합니다.

. 먼저, fsxadmin 사용자로 SSH를 통해 FSx 클러스터에 로그인하여 CLI를 통해 NFS 볼륨을 프로비저닝합니다.  AWS FSx ONTAP UI 콘솔에서 검색할 수 있는 FSx 클러스터 관리 IP 주소로 변경합니다.
+
[source, cli]
----
ssh fsxadmin@172.30.15.53
----
. 기본 Oracle VLDB 데이터베이스 데이터 파일 이미지 복사본을 저장하기 위해 기본 저장소와 동일한 크기의 NFS 볼륨을 만듭니다.
+
[source, cli]
----
...
----
. 또는 AWS FSx 콘솔 UI에서 다음 옵션을 사용하여 볼륨을 프로비저닝할 수 있습니다. 스토리지 효율성 `Enabled` , 보안 스타일 `Unix` , 스냅샷 정책 `None` 및 스토리지 계층화 `Snapshot Only` 아래에 표시된 대로.
+
image:aws-ora-fsx-ec2-vldb-vol.png["입력/출력 대화 상자 또는 서면 내용을 나타내는 그림"]

. 일일 일정과 30일 보존 기간을 적용한 Oracle 데이터베이스용 사용자 정의 스냅샷 정책을 만듭니다.  스냅샷 빈도와 보존 기간 측면에서 특정 요구 사항에 맞게 정책을 조정해야 합니다.
+
[source, cli]
----
...
----
. ec2-user로 EC2 인스턴스에 로그인하고 /nfsfsxn 디렉토리를 만듭니다.  추가 FSx 파일 시스템에 대한 추가 마운트 지점 디렉터리를 만듭니다.
+
[source, cli]
----
sudo mkdir /nfsfsxn
----
. FSx ONTAP NFS 볼륨을 EC2 DB 인스턴스 호스트에 마운트합니다.  FSx 가상 서버 NFS lif 주소로 변경하세요.  NFS lif 주소는 FSx ONTAP UI 콘솔에서 검색할 수 있습니다.
+
[source, cli]
----
sudo mount 172.30.15.19:/ora_01_copy /nfsfsxn -o rw,bg,hard,vers=3,proto=tcp,timeo=600,rsize=262144,wsize=262144,nointr
----
. 마운트 지점 소유권을 oracle:oisntall로 변경하고, 필요에 따라 oracle 사용자 이름과 기본 그룹으로 변경합니다.
+
[source, cli]
----
...
=== FSx에서 Oracle RMAN 증분 병합을 이미지 복사로 설정

[%collapsible%open]
====
RMAN 증분 병합은 모든 증분 백업/병합 간격마다 스테이징 데이터베이스 데이터 파일 이미지 복사본을 지속적으로 업데이트합니다.  데이터베이스 백업의 이미지 사본은 증분 백업/병합을 실행하는 빈도에 따라 최신 상태로 유지됩니다.  따라서 RMAN 증분 백업 및 병합 빈도를 결정할 때는 데이터베이스 성능, RTO 및 RPO 목표를 고려하세요.

. Oracle 사용자로 기본 DB 서버 EC2 인스턴스에 로그인합니다.
. Oracle 데이터 파일 이미지 복사본과 Oracle 플래시 복구 영역의 archlog 디렉토리를 저장하기 위해 마운트 지점 /nfsfsxn 아래에 oracopy 디렉토리를 만듭니다.
+
[source, cli]
----
...
----
. sqlplus를 통해 Oracle 데이터베이스에 로그인하고, 더 빠른 증분 백업을 위해 블록 변경 추적을 활성화하고, 현재 기본 스토리지에 있는 경우 Oracle 플래시 복구 영역을 FSx ONTAP 마운트로 변경합니다.  이를 통해 RMAN 기본 제어 파일/spfile 자동 백업 및 보관된 로그를 복구를 위해 FSx ONTAP NFS 마운트에 백업할 수 있습니다.
+
[source, cli]
----
sqlplus / as sysdba
...
----
. RMAN 백업 및 증분 병합 스크립트를 만듭니다.  스크립트는 병렬 RMAN 백업 및 병합을 위해 여러 채널을 할당합니다.  첫 번째 실행에서는 초기 전체 기준선 이미지 복사본이 생성됩니다.  전체 실행에서는 먼저 보존 기간이 지난 오래된 백업을 제거하여 스테이징 영역을 깨끗하게 유지합니다.  그런 다음 병합 및 백업 전에 현재 로그 파일을 전환합니다.  증분 백업은 병합 이후에 이루어지므로 데이터베이스 이미지 복사본은 현재 데이터베이스 상태보다 백업/병합 주기 하나 뒤처집니다.  사용자의 선호에 따라 병합 및 백업 순서를 바꾸어 더 빠르게 복구할 수 있습니다.  RMAN 스크립트는 기본 DB 서버의 crontab에서 실행되는 간단한 셸 스크립트로 통합될 수 있습니다.  RMAN 설정에서 제어 파일 자동 백업이 켜져 있는지 확인하세요.
+
....
...
....
. EC2 DB 서버에서 RMAN 카탈로그 유무에 관계없이 Oracle 사용자로 로컬로 RMAN에 로그인합니다.  이 데모에서는 RMAN 카탈로그에 연결하지 않습니다.
+
....
...
....
. RMAN 프롬프트에서 스크립트를 실행합니다.  첫 번째 실행에서는 기준 데이터베이스 이미지 복사본을 만들고, 이후 실행에서는 기준 이미지 복사본을 증분적으로 병합하여 업데이트합니다.  스크립트를 실행하는 방법과 일반적인 출력은 다음과 같습니다.  호스트의 CPU 코어에 맞게 채널 수를 설정합니다.
+
....
...
....
. 백업 후 데이터베이스 이미지 복사본을 나열하여 FSx ONTAP NFS 마운트 지점에 데이터베이스 이미지 복사본이 생성되었는지 확인합니다.
+
....
...
RMAN>
....
. Oracle RMAN 명령 프롬프트에서 스키마를 보고하여 현재 활성 데이터베이스 데이터 파일이 기본 스토리지 ASM +DATA 디스크 그룹에 있는지 확인합니다.
+
....
...
....
. OS NFS 마운트 지점에서 데이터베이스 이미지 복사본을 검증합니다.
+
....
...
이것으로 Oracle 데이터베이스 대기 이미지 복사본 백업 및 병합 설정이 완료되었습니다.

====


=== 빠른 복구를 위해 Oracle DB를 이미지 복사본으로 전환

[%collapsible%open]
====
데이터 손실이나 손상 등 기본 저장소 문제로 인한 장애가 발생하는 경우, 데이터베이스는 FSx ONTAP NFS 마운트의 이미지 복사본으로 빠르게 전환되고 데이터베이스 복원 없이도 현재 상태로 복구될 수 있습니다.  미디어 복원을 제거하면 VLDB의 데이터베이스 복구 속도가 엄청나게 빨라집니다.  이 사용 사례에서는 데이터베이스 호스트 인스턴스가 손상되지 않았고 데이터베이스 제어 파일, 보관된 로그 및 현재 로그를 모두 복구에 사용할 수 있다고 가정합니다.

. EC2 DB 서버 호스트에 Oracle 사용자로 로그인하고 전환하기 전에 테스트 테이블을 만듭니다.
+
....
...
....
. 종료 및 중단을 통해 데이터베이스를 실패로 시뮬레이션한 다음 마운트 단계에서 Oracle을 시작합니다.
+
....
SQL> shutdown abort;
...
....
. 마지막 증분 백업 이후 데이터베이스를 복구하고 열어 최신 상태로 업데이트합니다.
+
....
...
....
. 복구 후 sqlplus에서 데이터베이스 구조를 확인하여 제어, 임시 및 현재 로그 파일을 제외한 모든 데이터베이스 데이터 파일이 이제 FSx ONTAP NFS 파일 시스템에 복사되도록 전환되었는지 확인합니다.
+
....
...
SQL>
....
. SQL plus에서 복사하기 전에 삽입한 테스트 테이블의 내용을 확인합니다.
+
....
...
SQL>
....
. FSx ONTAP 고성능을 제공하는 중복 프로덕션 등급 스토리지이므로 성능 저하 없이 장기간 FSx NFS 마운트에서 Oracle 데이터베이스를 실행할 수 있습니다.  기본 저장소 문제가 해결되면 증분 백업 병합 프로세스를 역으로 수행하여 최소한의 가동 중지 시간으로 원래 상태로 돌아갈 수 있습니다.
...
[%collapsible%open]
====
기본 스토리지와 EC2 DB 인스턴스 호스트가 모두 손실되는 장애가 발생하면 원래 서버에서 복구를 수행할 수 없습니다.  다행히도 중복된 FSx ONTAP NFS 파일 시스템에 Oracle 데이터베이스 백업 이미지 사본이 남아 있습니다.  빠르게 동일한 EC2 DB 인스턴스를 프로비저닝하고 NFS를 통해 VLDB의 이미지 복사본을 새 EC2 DB 호스트에 쉽게 마운트하여 복구를 실행할 수 있습니다.  이 섹션에서는 이를 위한 단계별 절차를 보여드리겠습니다.

. 이전에 Oracle 데이터베이스를 대체 호스트 검증으로 복원하기 위해 생성한 테이블을 테스트하기 위해 행을 삽입합니다.
+
....
...
SQL>
....
. Oracle 사용자로서 RMAN 증분 백업을 실행하고 병합하여 트랜잭션을 FSx ONTAP NFS 마운트의 백업 세트로 플러시합니다.
+
....
[oracle@ip-172-30-15-99 ~]$ rman target / nocatalog
...
....
. 스토리지와 DB 서버 호스트의 전체 장애를 시뮬레이션하기 위해 기본 EC2 DB 인스턴스 호스트를 종료합니다.
. AWS EC2 콘솔을 통해 동일한 OS 및 버전을 사용하는 새로운 EC2 DB 인스턴스 호스트 ora_02를 보호합니다.  기본 EC2 DB 서버 호스트와 동일한 패치로 OS 커널을 구성하고, Oracle RPM을 사전 설치하고, 호스트에 스왑 공간도 추가합니다.  소프트웨어 전용 옵션으로 기본 EC2 DB 서버 호스트와 동일한 버전의 Oracle과 패치를 설치합니다.  이러한 작업은 아래 링크에서 제공되는 NetApp 자동화 툴킷을 사용하여 자동화할 수 있습니다.
+
툴킷:link:https://github.com/NetApp-Automation/na_oracle19c_deploy["na_oracle19c_deploy"^] 선적 서류 비치:link:../automation/marketing-overview.html#awxtower-deployments["NFS에서 Oracle19c for ONTAP 자동 배포"^]

. oratab, oraInst.loc, oracle 사용자 .bash_profile 등 기본 EC2 DB 인스턴스 호스트 ora_01과 유사하게 oracle 환경을 구성합니다.  해당 파일을 FSx ONTAP NFS 마운트 지점에 백업하는 것이 좋습니다.
. FSx ONTAP NFS 마운트의 Oracle 데이터베이스 백업 이미지 복사본은 중복성, 높은 가용성 및 고성능을 위해 AWS 가용성 영역에 걸쳐 있는 FSx 클러스터에 저장됩니다.  NFS 파일 시스템은 네트워크에 접속 가능한 한 새로운 서버에 쉽게 마운트할 수 있습니다.  다음 절차에서는 복구를 위해 Oracle VLDB 백업의 이미지 복사본을 새로 제공된 EC2 DB 인스턴스 호스트에 마운트합니다.
+
ec2-user로 마운트 지점을 생성합니다.
...
----
+
ec2-user로 Oracle VLDB 백업 이미지 복사본이 저장된 NFS 볼륨을 마운트합니다.
...
....
. 복구를 위해 FSx ONTAP NFS 마운트에서 사용 가능한 Oracle 보관 로그를 확인하고 마지막 로그 파일 로그 순서 번호를 기록해 둡니다.  이 경우에는 175입니다.  복구 지점은 로그 시퀀스 번호 176까지입니다.
+
....
...
....
. Oracle 사용자로서 ORACLE_HOME 변수를 새로운 EC2 인스턴스 DB 호스트 ora_02의 현재 Oracle 설치로 설정하고, ORACLE_SID를 기본 Oracle 인스턴스 SID로 설정합니다.  이 경우에는 db1입니다.
. Oracle 사용자로서 $ORACLE_HOME/dbs 디렉토리에 적절한 관리자 디렉토리를 구성하여 일반적인 Oracle init 파일을 만듭니다.  가장 중요한 것은 Oracle을 갖는 것입니다. `flash recovery area` 기본 Oracle VLDB 인스턴스에 정의된 FSx ONTAP NFS 마운트 경로를 가리킵니다.  `flash recovery area` 구성은 섹션에 설명되어 있습니다. `Setup Oracle RMAN incremental merge to image copy on FSx` .  Oracle 제어 파일을 FSx ONTAP NFS 파일 시스템으로 설정합니다.
+
[source, cli]
----
...
....
+
불일치가 있는 경우 위의 init 파일을 기본 Oracle DB 서버에서 복원된 백업 init 파일로 바꿔야 합니다.

. Oracle 사용자로서 RMAN을 실행하여 새로운 EC2 DB 인스턴스 호스트에서 Oracle 복구를 실행합니다.
+
....
...
....
. 데이터베이스 ID를 설정합니다.  데이터베이스 ID는 FSx NFS 마운트 지점의 이미지 복사본의 Oracle 파일 이름에서 검색할 수 있습니다.
+
....
...
....
. 자동 백업에서 제어 파일을 복원합니다.  Oracle 제어 파일과 spfile 자동 백업이 활성화된 경우, 이는 모든 증분 백업 및 병합 주기에서 백업됩니다.  여러 개의 사본이 있는 경우 최신 백업이 복원됩니다.
+
....
...
....
. 나중에 기본 DB 인스턴스와 일치하도록 매개변수 파일을 업데이트하기 위해 spfile에서 /tmp 폴더로 init 파일을 복원합니다.
+
....
...
....
. 제어 파일을 마운트하고 데이터베이스 백업 이미지 복사본을 검증합니다.
+
....
...
....
. 데이터베이스를 복원하지 않고 복구를 실행하려면 데이터베이스를 복사본으로 전환합니다.
+
....
RMAN> switch database to copy;
...
NOTE: 더 빠른 복구를 위해 recovery_parallelism 매개변수로 병렬 세션을 활성화하거나 데이터베이스 복구를 위한 복구 명령에서 병렬 정도를 지정하세요. `RECOVER DATABASE PARALLEL (DEGREE d INSTANCES DEFAULT);` .  일반적으로 병렬 처리 정도는 호스트의 CPU 코어 수와 같아야 합니다.

. RMAN을 종료하고 sqlplus를 통해 oracle 사용자로 Oracle에 로그인하여 데이터베이스를 열고 복구가 완료되지 않으면 로그를 재설정합니다.
+
....
...
....
. 기본 데이터베이스 장애가 발생하기 전에 삽입한 행이 있는 새 호스트에 복구된 데이터베이스를 검증합니다.
+
....
SQL> show pdbs
...
이로써 FSx ONTAP NFS 파일 시스템의 백업 이미지 복사본에서 새로운 EC2 DB 인스턴스 호스트로 Oracle VLDB 데이터베이스 복구가 완료되었습니다.

====


=== 다른 사용 사례를 위해 Oracle 대기 이미지 복사본을 복제합니다.

[%collapsible%open]
====
Oracle VLDB 이미지 복사를 스테이징하기 위해 AWS FSx ONTAP 사용하는 또 다른 이점은 최소한의 추가 스토리지 투자로 여러 다른 목적으로 FlexClon을 적용할 수 있다는 것입니다.  다음 사용 사례에서는 DEV, UAT 등과 같은 다른 Oracle 사용 사례를 위해 FSx ONTAP 에서 스테이징 NFS 볼륨의 스냅샷을 생성하고 복제하는 방법을 보여줍니다.

. 먼저, 이전에 만든 테스트 테이블에 행을 삽입합니다.
+
....
...
SQL>
....
. RMAN 백업을 수행하고 FSx ONTAP 데이터베이스 이미지 복사본에 병합하면 트랜잭션은 FSx NFS 마운트의 백업 세트에 캡처되지만 복제된 데이터베이스가 복구될 때까지 복사본에 병합되지 않습니다.
+
....
RMAN> @/home/oracle/rman_bkup_merge.cmd

....
. fsxadmin 사용자로 ssh를 통해 FSx 클러스터에 로그인하여 예약된 백업 정책(Oracle)에 의해 생성된 스냅샷을 관찰하고, 1단계에서 커밋한 트랜잭션을 포함하도록 일회성 스냅샷을 찍습니다.
+
....
...
....
. 대체 EC2 Oracle 호스트에서 새로운 DB1 복제 인스턴스를 구축하는 데 사용할 일회성 스냅샷에서 복제합니다.  볼륨 ora_01_copy에 대해 사용 가능한 일일 스냅샷에서 복제할 수 있는 옵션이 있습니다.
+
....
...
....
. 복제된 볼륨에 대한 스냅샷 정책을 끄세요. 복제된 볼륨은 부모 볼륨 스냅샷 정책을 상속하므로 복제된 볼륨을 보호하고 싶지 않으면 그대로 두세요.
+
....
...
....
. 기본 Oracle EC2 인스턴스와 동일한 버전 및 패치 수준이 사전 설치된 Oracle 소프트웨어가 있는 새 EC2 Linux 인스턴스에 로그인하고 복제된 볼륨을 마운트합니다.
+
....
...
....
. FSx NFS 마운트에서 데이터베이스 증분 백업 세트, 이미지 복사본 및 사용 가능한 보관 로그를 검증합니다.
+
....
[ec2-user@ip-172-30-15-124 ~]$ ls -ltr /nfsfsxn/oracopy
total 79450332
...
....
. 이제 복구 프로세스는 장애 발생 후 새 EC2 DB 인스턴스로 복구하는 이전의 사용 사례와 유사합니다. 기본 프로덕션 인스턴스와 일치하도록 Oracle 환경(oratab, $ORACLE_HOME, $ORACLE_SID)을 설정하고 FSx NFS 마운트의 플래시 복구 디렉토리를 가리키는 db_recovery_file_dest_size와 db_recovery_file_dest를 포함하는 init 파일을 만듭니다.  그런 다음 RMAN을 실행하여 복구를 실행합니다.  다음은 명령 단계와 출력입니다.
+
....
[oracle@ip-172-30-15-124 dbs]$ rman target / nocatalog
...
....
. Oracle nid 유틸리티를 사용하여 복제된 데이터베이스 인스턴스의 이름을 바꾸고 데이터베이스 ID를 변경합니다.  데이터베이스 인스턴스 상태는 다음과 같아야 합니다. `mount` 명령을 실행한다.
+
....
...
....
. oratab, init 파일에서 Oracle 데이터베이스 환경 구성을 새 데이터베이스 이름이나 인스턴스 ID로 변경하고 새 인스턴스 ID와 일치하는 필요한 관리 디렉터리를 만듭니다.  그런 다음 resetlogs 옵션을 사용하여 인스턴스를 시작합니다.
+
....
SQL> startup mount;
ORACLE instance started.
...
이것으로 DEV, UAT 또는 기타 사용 사례를 위한 FSx NFS 마운트의 스테이징 데이터베이스 복사본에서 새 Oracle 인스턴스의 복제가 완료됩니다.  여러 Oracle 인스턴스를 동일한 스테이징 이미지 복사본에서 복제할 수 있습니다.


NOTE: 오류가 발생하면 `RMAN-06571: datafile 1 does not have recoverable copy` 데이터베이스를 복사로 전환할 때 기본 프로덕션 DB와 일치하는 데이터베이스 모델을 확인하세요.  필요한 경우 RMAN 명령을 사용하여 기본과 일치하도록 화신을 재설정합니다. `reset database to incarnation n;` .
...
* RMAN: 병합된 증분 백업 전략(문서 ID 745798.1)
+
