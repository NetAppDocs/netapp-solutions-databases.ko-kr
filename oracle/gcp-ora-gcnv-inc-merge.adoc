...
---
= TR-5007: Google Cloud NetApp Volumes 에서 서버 내 증분 병합을 통해 Oracle VLDB 백업 및 복구 최적화
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Oracle Recovery Manager(RMAN) 백업 도구를 사용하여 Oracle에서 매우 큰 데이터베이스(VLDB)를 복구하는 것은 매우 어려운 작업이 될 수 있습니다.  장애 발생 시 백업 미디어에서 데이터베이스를 복원하는 과정은 시간이 많이 걸릴 수 있으며, 데이터베이스 복구가 지연되고 잠재적으로 서비스 수준 계약(SLA)에 상당한 영향을 미칠 수 있습니다.  비용 고려 사항으로 인해 RMAN 백업이 일반적으로 개체 스토리지로 스트리밍되기 때문에 클라우드에 구축된 Oracle VLDB의 상황은 더욱 악화되고 있습니다.  결과적으로 SLA가 필수적인 경우 VLDB를 복원하고 복구하는 것은 불가능합니다.  Oracle은 10g 버전부터 사용자가 DB 서버 호스트에 있는 추가 디스크 스토리지에 Oracle 데이터베이스 데이터 파일의 단계적 이미지 복사본을 생성할 수 있는 RMAN 기능을 도입했습니다.  이러한 이미지 복사본은 사용자가 정의한 간격으로 RMAN을 사용하여 증분적으로 업데이트할 수 있습니다.  장애가 발생한 경우, 데이터베이스 관리자(DBA)는 장애가 발생한 미디어에서 이미지 복사본으로 Oracle 데이터베이스를 신속하게 전환할 수 있으므로 데이터베이스 미디어를 완전히 복원할 필요가 없습니다.  결과적으로 SLA는 크게 향상되었지만, 필요한 데이터베이스 저장 용량은 두 배로 늘어나게 됩니다.

VLDB에 대한 SLA를 원하고 Google Cloud Platform(GCP)과 같은 퍼블릭 클라우드로 Oracle 데이터베이스를 옮기는 것을 고려하고 있다면, Google Cloud NetApp Volumes (GCNV)와 같은 리소스를 사용하여 스탠바이 데이터베이스 이미지 복사본을 스테이징하는 유사한 데이터베이스 보호 구조를 설정할 수 있습니다.  GCNV는 Google에서 제공하는 완전 관리형 고성능 파일 저장 서비스로, Google Cloud에서 Oracle 데이터베이스와 같은 까다로운 엔터프라이즈 애플리케이션을 보다 쉽고, 안전하고, 비용 효율적으로 마이그레이션하고 실행할 수 있도록 해줍니다.  다양한 서비스 수준을 통해 저장 비용을 제어하는 유연성을 제공하는 동시에 거의 0에 가까운 RPO와 관리 가능한 RTO를 달성할 수 있습니다.  이 문서에서는 GCNV 스토리지 풀에서 NFS 파일 시스템을 프로비저닝하고 내보내 Oracle 데이터베이스 서버에 마운트하여 기본 스토리지 장애 발생 시 신속한 복구를 위해 대기 데이터베이스 복사본을 준비하는 방법을 보여줍니다.
...
이 솔루션의 테스트와 검증은 최종 배포 환경과 일치하지 않을 수 있는 Google Cloud NetApp Volumes 스토리지 풀과 Google Compute Engine VM에서 수행되었습니다. 자세한 내용은 다음 섹션을 참조하세요. <<배포 고려사항의 핵심 요소>> .



=== 아키텍처

image:gcp-ora-gcnv-inc-merge-architecture.png["이 이미지는 GCNV를 사용하여 Google Cloud에서 Oracle VLDB 증분 병합을 구현한 자세한 그림을 제공합니다."]
...
| DB 서버용 Google Compute Engine VM | N1(4개 vcpu, 15GiB 메모리) | 2개의 VM, 하나는 기본 DB 서버로, 다른 하나는 대기 또는 복제 DB 서버로 사용 
...
* *GCNV 스토리지 풀 서비스 수준.*  GCNV 스토리지 풀은 Standard, Premium, Extreme, Flex의 4가지 서비스 수준을 제공합니다.  기본적으로 자동 QoS는 스토리지 풀 내에서 생성된 볼륨에 적용되며, 볼륨의 크기와 서비스 수준에 따라 볼륨의 처리량을 제한합니다.  유일한 예외는 Flex 스토리지 풀에서 할당된 볼륨이 스토리지 풀에서 사용 가능한 처리량을 공유한다는 것입니다.
* *RMAN 증분 병합을 위한 Oracle VLDB 서버 내 백업 저장 용량 한도.*  테스트와 검증 과정에서 Oracle 증분 백업 및 병합을 위한 NFS 볼륨은 표준 서비스 수준의 단일 GCNV 스토리지 풀에서 할당되며, 이 스토리지 풀의 용량 제한은 200TiB입니다.  해당 임계값을 초과하여 배포하는 경우, 여러 개의 GCNV 스토리지 풀을 여러 NFS 마운트 지점과 병렬로 구현하여 더 높은 용량을 제공할 수 있습니다.
* *RMAN 증분 병합을 사용한 Oracle 복구성.*  RMAN 서버 내 증분 백업 및 병합은 일반적으로 RTO 및 RPO 목표에 따라 사용자가 정의한 빈도로 실행됩니다.  기본 데이터베이스 파일 저장소 및/또는 보관된 로그가 완전히 손실되면 데이터 손실이 발생할 수 있습니다.  Oracle 데이터베이스는 서버 내 데이터베이스 백업 이미지 복사본에서 사용 가능한 마지막 증분 백업까지 복구할 수 있습니다.  데이터 손실을 최소화하려면 Oracle 플래시 복구 영역을 GCNV NFS 백업 마운트 지점에 설정해야 하며, 보관된 로그는 장애 발생 시 데이터베이스를 마지막 보관된 로그로 롤포워드하기 위해 데이터베이스 이미지 복사본과 함께 동일한 NFS 마운트에 백업해야 합니다.  따라서 거의 0에 가까운 RPO를 달성할 수 있습니다.
* *GCNV 스토리지 풀에서 NFS 이미지 백업을 사용하여 Oracle VLDB를 실행합니다.*  이 시점에서는 데이터베이스 볼륨 서비스 수준을 표준에서 프리미엄 또는 익스트림으로 재할당하는 것이 지원되지 않습니다.  그러나 서비스 수준을 프리미엄에서 익스트림으로 재할당하는 것은 지원됩니다.  장애 발생 후 장기간 이미지 복사본으로 데이터베이스를 실행하려면 프리미엄 스토리지 풀에서 볼륨을 할당하여 데이터베이스 이미지 복사본 백업을 수행하는 것이 좋습니다.
* *Google Compute Engine VM.*  테스트 및 검증에서는 N1 유형의 GCE VM 인스턴스를 Oracle 데이터베이스 서버로 사용했습니다.  데이터베이스 작업 부하에 더 적합하고 최적화된 다른 유형의 GCE VM도 있습니다.  실제 작업 부하 요구 사항에 따라 vCPU 수와 RAM 양에 맞게 VM 크기를 적절히 조정합니다.
* *dNFS 구성.* dNFS는 Oracle 커널에 내장되어 있으며 Oracle을 NFS 스토리지에 배포할 때 Oracle 데이터베이스 성능을 획기적으로 높이는 것으로 알려져 있습니다. dNFS는 Oracle 바이너리에 패키징되어 있지만 기본적으로 켜져 있지 않습니다.  NFS에 Oracle 데이터베이스를 배포하는 경우 이 기능을 켜야 합니다.  VLDB에 대한 여러 GCNV 스토리지 풀 배포의 경우, 다양한 GCNV 스토리지 풀에 대한 dNFS 다중 경로를 적절하게 구성해야 합니다.
...
이 솔루션은 Oracle VLDB가 VPC 내의 Google Cloud 환경에 이미 배포되어 있다고 가정합니다.  Oracle VLDB는 GCNV 스토리지나 Google Cloud 생태계 내의 다른 스토리지 옵션에서 실행될 수 있습니다.  다음 섹션에서는 RMAN을 통해 GCNV 스토리지 풀에서 마운트된 NFS 파일 시스템에 Oracle VLDB 이미지 백업 및 증분 병합을 설정하는 단계별 배포 절차를 제공합니다.  Oracle VLDB도 다른 GCNV 스토리지 풀에 호스팅된 기본 스토리지를 갖고 있습니다.
...
. Google Cloud 포털 콘솔에서 설명서를 따르세요.link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/storage-pools/create-storage-pool["스토리지 풀 생성"^] 아래와 같이 표준 서비스로 GCNV 스토리지 풀을 생성합니다.
+
image:gcp-ora-gcnv-inc-merge-pool-001.png["이 이미지는 Google Cloud Portal 콘솔의 GCNV 스토리지 풀 화면을 보여줍니다."]

. Google Cloud 포털 콘솔에서 설명서를 따르세요.link:https://cloud.google.com/netapp/volumes/docs/configure-and-use/volumes/create-volume["볼륨을 생성합니다"^] 서버 내 Oracle VLDB 이미지 백업을 위한 볼륨을 생성합니다.  이 솔루션 데모에서는 Oracle VLDB 기본 저장소도 아래와 같이 GCNV에 호스팅됩니다.
+
image:gcp-ora-gcnv-inc-merge-vol-002.png["이 이미지는 Google Cloud Portal 콘솔의 GCNV 스토리지 볼륨 화면을 보여줍니다."]
...
+
image:gcp-ora-gcnv-inc-merge-vol-003.png["이 이미지는 Google Cloud Portal 콘솔의 GCNV 스토리지 볼륨 화면을 보여줍니다."]

. 기본 Oracle VLDB 서버에 로그인합니다.  루트로서 GCNV 저장소에서 내보낸 NFS 볼륨을 마운트합니다.  필요에 따라 NFS 서버 IP 주소와 파일 경로를 변경하세요.
+
[source, cli]
----
sudo mkdir /nfsgcnv
...
. 이 솔루션 검증을 위해 기본 데이터베이스는 그리드 인프라 재시작 구성을 갖춘 독립형 서버로 설정됩니다.  기본 저장소는 프리미엄 서비스를 제공하는 GCNV 스토리지 풀에 호스팅됩니다.  다시 말하지만, 이는 필수 사항이 아니며 기본 저장소는 Google Cloud의 다른 저장소 옵션에 있을 수 있습니다.
+
....
...
----
. sqlplus를 통해 Oracle 데이터베이스에 로그인하고, 더 빠른 증분 백업을 위해 블록 변경 추적을 활성화하고, 현재 기본 스토리지에 있는 경우 Oracle 플래시 복구 영역을 GCNV NFS 마운트로 변경합니다.  이를 통해 RMAN 기본 제어 파일/spfile 자동 백업 및 보관 로그를 GCNV NFS 마운트에 백업하여 마지막으로 사용 가능한 보관 로그로 데이터베이스를 복구할 수 있습니다.
+
[source, cli]
----
sqlplus / as sysdba
...
....
. 백업 후 데이터베이스 이미지 복사본을 나열하여 GCNV NFS 마운트 지점에 데이터베이스 이미지 복사본이 생성되었는지 확인합니다.
+
....
...
[%collapsible%open]
====
데이터 손실이나 손상 등 기본 저장소 문제로 인한 장애가 발생하는 경우, 데이터베이스는 GCNV NFS 마운트의 이미지 복사본으로 빠르게 전환되고 데이터베이스 복원 없이도 현재 상태로 복구될 수 있습니다.  미디어 복원을 제거하면 VLDB의 데이터베이스 복구 속도가 엄청나게 빨라집니다.  이 사용 사례에서는 Oracle VLDB DB 서버가 손상되지 않았고 데이터베이스 제어 파일, 보관된 로그 및 현재 로그를 모두 복구할 수 있다고 가정합니다.
...
....
. 테스트 테이블에 행을 삽입합니다.  행은 가장 최근의 증분 백업 및 병합 이후에 삽입되었으며 해당 데이터는 백업 세트에 없습니다.  이후 데모에서는 데이터베이스가 완전히 복구된 후 테스트 행이 복구되는 것을 보여줍니다.
+
....
...
....
. 복구 후 sqlplus에서 데이터베이스 구조를 확인하여 제어, 임시 및 현재 로그 파일을 제외한 모든 VLDB 데이터 파일이 이제 GCNV NFS 파일 시스템에 복사되도록 전환되었는지 확인합니다.
+
....
...
[%collapsible%open]
====
기본 저장소에서 저장소 오류가 해결되면 최소한의 가동 중지 시간으로 데이터베이스를 쉽게 기본 저장소로 되돌릴 수 있습니다.  단계별 절차는 다음과 같습니다.
...
....
. RMAN 백업 및 병합 스크립트를 수정하여 기본 스토리지를 가리키고, 테스트 케이스에서는 RMAN 채널 정의에 ASM 디스크 그룹 +DATA를 지정합니다.  수정된 백업 스크립트를 실행하여 기본 저장소에 새 이미지 사본을 만듭니다.
+
....
...
....
. 마지막으로, GCNV 백업 저장소에 있는 오래된 DB 이미지 복사본을 삭제하고 GCNV 백업 저장소에 새로운 백업 이미지를 만들어 주기를 완료합니다.
...
[%collapsible%open]
====
기본 저장소 및/또는 기본 DB 서버 호스트가 손실되는 장애가 발생하는 경우, 보관된 로그 파일에 없는 모든 트랜잭션은 손실됩니다.  하지만 GCNV NFS 파일 시스템에서 사용할 수 있는 Oracle 데이터베이스 백업 이미지 사본이 유용합니다.  백업 이미지 복사본을 사용하면 기본 데이터베이스를 동일한 기본 데이터베이스 서버 또는 사용 가능한 경우 대기 데이터베이스 서버로 빠르게 복구할 수 있습니다.  이 섹션에서는 기본 DB 서버로 복구하는 단계별 절차를 보여드리겠습니다.  새로운 스탠바이 DB 서버로 복구하는 방법은 거의 동일합니다. 그러나 데이터베이스 백업 이미지 사본은 동일한 Oracle 소프트웨어 스택이 설치 및 구성된 스탠바이 DB 서버에 마운트되어야 합니다.
...
. 대기 DB 서버에서 복구를 수행하는 경우, 해당 서버가 기본 DB 서버 호스트와 동일한 OS, 동일한 버전이며, OS 커널이 패치되었는지 확인하세요.  또한, 동일한 버전의 Oracle과 패치가 소프트웨어 전용 옵션으로 스탠바이 DB 서버에 설치 및 구성되었습니다.
...
. GCNV NFS 파일 시스템의 Oracle 데이터베이스 백업 이미지 복사본은 복구를 위해 대기 DB 서버에 마운트됩니다.  다음 절차는 프로세스 세부 정보를 보여줍니다.
+
관리자로서 마운트 지점을 생성합니다.
...
----
. 복구를 위해 GCNV NFS 마운트에서 사용 가능한 Oracle 보관 로그를 확인하고 마지막 로그 파일 로그 순서 번호를 기록해 둡니다.  우리의 테스트 케이스에서는 27이었습니다.  복구 지점은 로그 시퀀스 번호 28까지입니다.
. 대기 DB 호스트에서 복구하는 경우 Oracle 사용자로 ORACLE_HOME 변수를 대기 DB 서버의 현재 Oracle 설치로 설정하고 ORACLE_SID를 기본 Oracle 인스턴스 SID로 설정합니다.  이 테스트에서는 NTAP입니다.
+
....
...
....
. Oracle 사용자로서 $ORACLE_HOME/dbs 디렉토리에 적절한 관리자 디렉토리를 구성하여 일반적인 Oracle init 파일을 만듭니다.  가장 중요한 것은 Oracle을 갖는 것입니다. `flash recovery area` 기본 Oracle VLDB 서버에 정의된 GCNV NFS 마운트 경로를 가리킵니다.  `flash recovery area` 구성은 섹션에 설명되어 있습니다. `Setup Oracle RMAN incremental merge to image copy on GCNV for in-server VLDB backup` .  Oracle 제어 파일을 GCNV NFS 백업 파일 시스템으로 설정합니다.
+
[source, cli]
----
...
. Oracle 사용자로서 RMAN을 실행하여 기본 또는 대기 DB 서버 호스트에서 Oracle 복구를 실행합니다.  먼저 Oracle 인스턴스를 시작합니다. `nomount` 상태.
+
....
...
....
. 데이터베이스 ID를 설정합니다.  데이터베이스 ID는 GCNV NFS 마운트 지점의 이미지 복사본의 Oracle 파일 이름에서 검색할 수 있습니다.
+
....
...
....
. 기본 저장소로 돌아가려면 섹션에 자세히 설명된 것과 정확히 동일한 절차를 따르십시오. `Reverse VLDB back to primary storage from backup copy` .  대기 DB 서버에서 복구하는 경우 기본 스토리지를 대기 DB 서버로 가져와야 합니다.  또한 GCNV nfs 마운트에서 Oracle 제어 파일과 현재 온라인 로그 파일을 기본 스토리지로 다시 옮겨야 할 필요성이 있는데, 이는 몇 분 정도의 다운타임으로 완료할 수 있습니다.
...
[%collapsible%open]
====
GCNV의 백업 이미지 사본에서 새로운 Oracle VLDB를 복제하는 것은 기본 데이터베이스를 새로운 DB 서버로 특정 시점으로 복구하는 것과 비슷합니다.  NetApp 솔루션 엔지니어링 팀은 빠르고(분) 간편한 구현을 위해 전체 프로세스를 원활하게 진행할 수 있는 자동화 툴킷을 제공합니다.

다음 자동화 툴킷에는 NFS 마운트를 통해 GCNV 스토리지에 저장된 기본 데이터베이스 백업 이미지 사본에서 Oracle VLDB의 복제본을 만들거나 새로 고치는 데 사용할 수 있는 플레이북이 포함되어 있어 완전한 복제 수명 주기를 관리할 수 있습니다.  자세한 내용은 다음 문서를 참조하세요.link:automation-ora-bkup-clone.html["서버 내 이미지 백업을 통한 자동화된 Oracle VLDB 복제 수명 주기"] .
